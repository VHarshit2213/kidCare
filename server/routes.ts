...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/babysitters", async (_req, res) => {
    try {
      ...
      const babysittersWithoutPasswords = availableBabysitters.map(
        (babysitter) => {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to get babysitters" });
    }
  });
    ...
    try {
      ...
      const nearbyBabysitters = allBabysitters
        .filter((sitter) => {
          ...
          if (requiresFirstAid === "true" && !sitter.firstAidCertified)
            return false;
          if (requiresTransportation === "true" && !sitter.hasTransportation)
            return false;
          ...
          if (
            minExperience &&
            (!experience || experience < parseInt(minExperience as string))
          )
            return false;
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to get nearby babysitters" });
    }
  });
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (user.userType !== "parent") {
          return res
            .status(403)
            .json({ message: "Only parents can create bookings" });
        }
  ...
  app.post("/api/instant-bookings", async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to create instant booking" });
    }
  });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.userType !== "parent") {
          return res
            .status(403)
            .json({ message: "Only parents can view their bookings" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.userType !== "babysitter") {
          return res
            .status(403)
            .json({ message: "Only babysitters can view their bookings" });
        }
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      ...
      if (
        !status ||
        !["pending", "accepted", "completed", "cancelled"].includes(status)
      ) {
      ...
      try {
        ...
        if (
          ...
          return res
            .status(403)
            .json({ message: "You can only update bookings assigned to you" });
        }
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (
          user.userType !== "babysitter" ||
          user.id !== Number(babysitterId)
        ) {
          return res.status(403).json({
            message: "Only babysitters can assign themselves to bookings",
          });
        }
        ...
        if (booking.babysitterId) {
          return res
            .status(409)
            .json({ message: "Booking is already assigned to a babysitter" });
        }
        ...
        if (updatedBooking) {
          try {
            // Get both the parent and babysitter details for the notification
            const parent = await storage.getUser(updatedBooking.parentId);
            const babysitter = updatedBooking.babysitterId
              ? await storage.getUser(updatedBooking.babysitterId)
              : null;
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          !notificationType ||
          !["sms", "call", "both"].includes(notificationType)
        ) {
          return res.status(400).json({
            message:
              "Invalid notification type. Must be 'sms', 'call', or 'both'.",
          });
        }
        ...
        if (
          ...
          return res.status(403).json({
            message:
              "You can only send notifications for bookings assigned to you",
          });
        }
        ...
        if (!booking.babysitterId) {
          return res.status(400).json({
            message:
              "Cannot send notifications for bookings without an assigned babysitter",
          });
        }
        ...
        if (!parent || !babysitter) {
          return res.status(500).json({
            message: "Could not retrieve user details for notifications",
          });
        }
  ...
  app.post(
    "/api/bookings/:id/masked-message",
    authenticate,
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (currentUser.id === booking.parentId) {
          ...
          return res
            .status(403)
            .json({ message: "You are not associated with this booking" });
        }
        ...
        const result = await sendMaskedSMS(
          fromUser,
          toUser,
          bookingId,
          message
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (result.success) {
          ...
          res
            .status(500)
            .json({ message: result.error || "Failed to initiate call" });
        }
      } catch (error: any) {
        res
          .status(500)
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (data.senderId !== user.id) {
          return res
            .status(403)
            .json({ message: "Sender ID must match authenticated user" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.id !== requestedUserId) {
          return res
            .status(403)
            .json({ message: "You can only view your own messages" });
        }
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.userType !== "parent") {
          return res.status(403).json({
            message: "Only parents can update their profile with this endpoint",
          });
        }
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.userType !== "babysitter") {
          return res.status(403).json({
            message: "Only babysitters can update their availability",
          });
        }

        if (
          !availabilityStatus ||
          !["available", "offline", "busy"].includes(availabilityStatus)
        ) {
          return res.status(400).json({
            message:
              "Invalid availability status. Must be 'available', 'offline', or 'busy'",
          });
        }
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          !reviewStatus ||
          !["approved", "rejected", "pending"].includes(reviewStatus)
        ) {
          return res.status(400).json({
            message:
              "Valid review status required (approved, rejected, or pending)",
          });
        }
        ...
        if (targetUser.userType !== "babysitter") {
          return res
            .status(400)
            .json({ message: "Review status can only be set for babysitters" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.userType !== "parent") {
          return res
            .status(403)
            .json({ message: "Only parents can add children" });
        }
        ...
        if (
          ...
          typeof requestBody.dateOfBirth === "string"
        ) {
        ...
        if (data.parentId !== user.id) {
          return res
            .status(403)
            .json({ message: "Parent ID must match authenticated user" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.userType !== "parent") {
          return res
            .status(403)
            .json({ message: "Only parents can view their children" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (child.parentId !== user.id) {
          return res.status(403).json({
            message: "You can only update your own children's information",
          });
        }
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (child.parentId !== user.id) {
          return res.status(403).json({
            message: "You can only view your own children's information",
          });
        }
  ...
  if (process.env.STRIPE_SECRET_KEY) {
    ...
    app.post(
      "/api/bookings/:id/create-payment",
      authenticate,
      async (req: Request, res: Response) => {
        try {
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          if (paymentIntent.status !== "succeeded") {
            return res
              .status(400)
              .json({ message: "Payment has not succeeded" });
          }
  ...
  if (process.env.STRIPE_SECRET_KEY) {
    ...
    > = {
      BLKFDEAL25: {
        ...
        end: Date.UTC(2025, 10, 29, 21, 0, 0), // Nov 29, 2025 1:00 PM PST
      },
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          if (!paymentType || !userId) {
            return res
              .status(400)
              .json({ message: "Payment type and user ID are required" });
          }
          ...
          if (promoCode && discount) {
            ...
            if (promo.end && now > promo.end) {
              return res.status(400).json({
                message: "Promo expired. Ended Nov 29, 2025, 1:00 PM PST.",
              });
            }
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const membershipStatus =
            paymentType === "full" ? "active" : "installment_1";
          ...
          try {
            await storage.updateUserMembership(Number(userId), {
              membershipStatus,
              membershipType:
                paymentType === "full" ? "one-time" : "installment",
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          if (!userId || !membershipStatus) {
            return res
              .status(400)
              .json({ message: "User ID and membership status are required" });
          }
          ...
          if (
            ...
            return res
              .status(400)
              .json({ message: "Invalid membership status" });
          }
          ...
          const metadata: any = {
            membershipStatus,
            membershipType:
              membershipStatus === "active" ? "one-time" : "installment",
            membershipPaymentDate: new Date(),
          };
          ...
          // const updatedUser = await storage.updateUserMembership(
          //   Number(userId),
          //   metadata,
          // );
          ...
          // res.status(200).json(userWithoutPassword);
          res.status(200).json("taken membership succesfull");
        } catch (error: any) {
          res.status(500).json({
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        // Only babysitters can create reviews
        if (user.userType !== "babysitter") {
          return res
            .status(403)
            .json({ message: "Only babysitters can create reviews" });
        }
        ...
        // Validate that the booking exists and babysitter was assigned to it
        const booking = await storage.getBooking(reviewData.bookingId);
        if (!booking) {
          return res.status(404).json({ message: "Booking not found" });
        }

        if (booking.babysitterId !== user.id) {
          return res.status(403).json({
            message: "You can only review bookings you were assigned to",
          });
        }
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const reviews = await storage.getReviewsByRevieweeId(parentId);
        res.json(reviews);
      } catch (error: any) {
        res
          .status(500)
      ...
      try {
        ...
        const reviewsWithUserInfo = await Promise.all(
          reviews.map(async (review) => {
            const parent = await storage.getUser(review.revieweeId);
            const babysitter = await storage.getUser(review.reviewerId);
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.userType !== "parent") {
          return res
            .status(403)
            .json({ message: "Only parents can create reviews" });
        }
  ...
  app.get(
    "/api/parent-reviews/booking/:bookingId",
    authenticate,
    async (req: Request, res: Response) => {
      try {
        const bookingId = parseInt(req.params.bookingId);
