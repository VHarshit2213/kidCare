...
export default function AvailableSittersPopup({ 
  ...
  const availableSitters: (User & { distance: number })[] = allSitters
    ...
    .filter((sitter: User & { distance: number }) => 
      // Filter sitters within 8 miles
      sitter.distance <= 8
    )
    .sort((a: User & { distance: number }, b: User & { distance: number }) => 
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          ...
          <DialogDescription>
            We found {availableSitters.length} sitters available within 8 miles for your request
          </DialogDescription>
        </DialogHeader>
